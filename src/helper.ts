 ...
 * @return bool
 */
function isResourceNotFoundError(error: unknown): error is ResourceNotFoundException {
  return !!(error
    && typeof error === 'object'
    && 'name' in error
...
function getCredentials(ci = process.env.CI) {
  ...
  return fromIni({
    mfaCodeProvider: async (serial: string) => await prompt(
      `Type the mfa token for the following account: ${ serial }\n`
    ),
  });
}
